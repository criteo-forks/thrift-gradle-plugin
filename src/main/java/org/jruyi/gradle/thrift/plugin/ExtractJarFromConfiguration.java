...
public class ExtractJarFromConfiguration {
  ...
  public static ImmutableSet<File> makeThriftPathFromJars(File temporaryThriftFileDirectory, Iterable<File> classpathElementFiles)
      throws IOException {
      ...
      if (classpathElementFile.isFile() && classpathElementFile.canRead() &&
        ...
        for (JarEntry jarEntry : list(classpathJar.entries())) {
          ...
          if (jarEntry.getName().endsWith(THRIFT_FILE_SUFFIX)) {
            ...
            /* First modification: when we have directories in the jar
             * contains directories itself we have an issue in the maven plugin.
             ...
             * That's why we add the second version here
             */
  ...
  /**
   * Second modification: gradle does not take jars from local repositories, it can be found either
   ...
   * the name of the jar if jarPath ends with .jar and raise an exception else
   * @param jarPath the full path of a jar file.
   * @return the truncated path relative to the local repository or root of the drive.
   */
  private static String truncatePath(final String jarPath) {
    if (jarPath.endsWith(".jar")) {
      return Paths.get(jarPath).getFileName().toString();
    } else {
      throw new RuntimeException("Does not support jar files that do not end with .jar");
    }
  }
}
